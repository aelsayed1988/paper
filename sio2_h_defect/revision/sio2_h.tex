...
In this paper, we calculate a number of different properties of hydrogen induced defects in a-SiO$_2$. For some of these properties, we have obtained a large data set, and where appropriate we use statistical descriptors to give the reader an idea of the shapes of the distributions we obtain. However, some properties' data set was rather limited, such as the calculated barriers, and we do not attempt to give a statistical view of these data. It is important to note that while we use statistical descriptors, it is merely to give the reader an idea of the data that we have obtained. These statistics are not meant to be representative of the defects' entire population in a-SiO$_2$, but offer a statisticali view of the defects that we have studied using the methods described above and further geometrical constraints described in each defects' section in the paper. 
...
Remarkably, we find that 15 out of 50 of configurations of the hydroxyl E$^\prime$ center are thermodynamically stable in the neutral charge state over a range of Fermi levels, in contrast to what has been calculated for interstitial hydrogen in the literature.~\cite{godet_hydrogen,robertson_oxides}.  The formation energy as a function of the Fermi level position for a single defect configuration where the neutral configuration is stable across the Si band gap is shown in Fig. \ref{fig:hydroxy_thermodynamic}b). The average formation energy for the neutral configuration with the Fermi level lying at the top of the a-SiO$_2$ valence band is 2.19 eV, ranging from 1.95 to 3.27 eV and with a standard deviation of $2.2 \times 10^{-1}$ eV. 
